...
function checkForm(){
	//判断用户名不能为空
	if($("texNameId").value.length==0){
		$("username").innerHTML = "用户名不能为空！";
		$("texNameId").focus();
		$("username").style.color="red";
     	$("username").style.fontWeight="bold";
		return false;
		}else if($("texNameId").value.length>=5){
		$("username").innerHTML = "用户名不能大于5个字符！";
		$("username").style.color="red";
     	$("username").style.fontWeight="bold";
     	$("texNameId").focus();
		$("texNameId").select();
...
function checkTelphone(){	
		if($("texTelphoneId").value.length==0){//验证手机号码是否为空
		$("userTelphone").innerHTML = "手机号码不能为空！";
		$("texTelphoneId").focus();
		$("userTelphone").style.color="red";
     	$("userTelphone").style.fontWeight="bold";
		return false;
		}else{
			var patt=/^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,12})+$/;
			if(patt.test($("texTelphoneId").value)){//验证手机号码格式是否正确
				$("userTelphone").innerHTML = " ";
				return true;
			}else{
				$("userTelphone").innerHTML = "手机号码格式不正确！";
				$("texTelphoneId").focus();
				$("texTelphoneId").select();
				$("userTelphone").style.color="red";
				$("userTelphone").style.fontWeight="bold";
		}
...
function checkOffice(){
	var patrn=/^[+]{0,1}(\d){1,3}[ ]?([-]?(\d){1,12})+$/;
			//办公电话号码格式是否正确
		if(patrn.test($("texOfficeId").value)){
		$("userOffice").innerHTML = " ";
		}else{
		$("userOffice").innerHTML = "电话号码格式不正确！";
		$("userOffice").style.color="red";
     	$("userOffice").style.fontWeight="bold";
		}
}		
...
function checkHome(){
		var patrn=/^[+]{0,1}(\d){1,3}[ ]?([-]?(\d){1,12})+$/;
		if(patrn.test($("texHomeId").value)){//家庭电话号码格式是否正确
			$("userHome").innerHTML = " ";
		}else{
			$("userHome").innerHTML = "电话号码格式不正确！";
			$("texHomeId").focus();
			$("texHomeId").select();
			$("userHome").style.color="red";
	     	$("userHome").style.fontWeight="bold";
		}
}
	
function checkmm(){
		if($("texAddressId").value.length>=10){
		alert("地址内容过长！");
		return false;
		}else if($("texAddressId").value.length>=10){
		alert("内容过长！");
		return false;
		}
		return true;
